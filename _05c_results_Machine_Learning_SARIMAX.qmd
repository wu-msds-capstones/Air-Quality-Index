...
This will help us to understand the seasonality and trend clearly and allowing us to make more sense of the data for the forecast.  
...
We define p,d,q hyperparameters to take any value between 0 to 1. 
These include the p, d, and q parameters, which help the model decide how much weight to give to past values, trends, and patterns. We then create a list of all p,d,q possible combinations of these settings in a for loop.
...
### Interpretation
The table above summarizes the results of the SARIMAX model applied to the air quality index data. The model and its corresponding criteria (AIC, BIC, HQIC) provide insight into the model's fit and complexity.
...
Next, we extract the coefficient summary  

| **Coefficient Table** | **coef** | **std err** | **z** | **P>|z|** | **[0.025** | **0.975]** |
|:--------------------|:---------|:------------|:--------|:----------|:-----------|:-----------|
| **ar.L1**           | 0.0656   | 0.246       | 0.267   | 0.789    | -0.416     | 0.547     |
| **ma.L1**           | -0.9993  | 7.412       | -0.135  | 0.893    | -15.527    | 13.529    |
| **ma.S.L12**        | -0.9999  | 263.482     | -0.004  | 0.997    | -517.416   | 515.416   |
| **sigma2**          | 115.3748 | 3.03e+04    | 0.004   | 0.997    | -5.93e+04  | 5.96e+04  |

: SARIMAX Coefficient Summary {.striped .hover}

### Interpretation

The table above summarizes the coefficients, standard errors, z-values, p-values, and 95% confidence intervals for the SARIMAX model parameters. This information is crucial for evaluating the significance and reliability of each parameter estimate
...
Our conclusion is that the SARIMAX model did not predict well the forecast values of the air quality index (AQI). As mentioned in the plot diagnostics, the high p-value and z-score close to zero indicate that the model's hyperparameters may not be significantly contributing to the prediction accuracy.
...
- *External Influences*: The SARIMAX model may not account for all external factors influencing AQI, such as sudden changes in weather conditions, local events, or policy changes affecting pollution levels.