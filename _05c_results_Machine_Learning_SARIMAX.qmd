...
For this project, we used an extended version of the *ARIMA* model knows as the *SARIMAX* model as explained in the methods section. We use a tuning technique called grid search that attempts to compute the optimum values of hyperparameters. We are trying to find the right p,d,q values that would be given as an input to the SARIMAX time series model.
